...
export type GameContext = {
  ...
  selectResourceChoice: (options: Array<Partial<ResourceMap>>, totalLevel: number, rawInput?: boolean) => Promise<Partial<ResourceMap> | null>;
...
export const cardEffectsRegistry: Record<number, Record<number, CardEffect[]>> = {
    ...
    3: [{ // Inventrice Inspirée
      ...
      execute: async function (ctx) {
        ...
        if (choice.includes('Resources')) {
          const resourcesChoice = [{coin: checkedBoxes}, {wood: checkedBoxes}, {stone: checkedBoxes}, {metal: checkedBoxes}, {sword: checkedBoxes}, {tradegood: checkedBoxes}];
          
          const selectedResource = await ctx.selectResourceChoice(resourcesChoice, 1);
  ...
  53: {
    1: [
      ...
      },
      {
        ...
        timing: "onClick",
        execute: async function (ctx) {
          const selected = await ctx.selectCardsFromZone((card) => card.GetType(ctx.t).includes(ctx.t('person')), ctx.t('playArea'), this.description(ctx.t), 3, ctx.card, 0, 'person');
    ...
    1: [
      ...
      },
      {
        ...
        execute: async function (ctx) {
          ...
          selected = await ctx.selectCardsFromZone((card) => card.GetType(ctx.t).includes(ctx.t('person')), ctx.t('deck'), this.description(ctx.t), 2, ctx.card, 0, 'person');
          for (const card of selected) {
    ...
    3: [
      { // Soldat Ennemi
        ...
        execute: async function (ctx) {
          ...
          while (selectedCards.length === 0 && ctx.filterZone(ctx.zone, filter).length !== 0) {
            selectedCards = await ctx.selectCardsFromZone(filter, ctx.t('playArea'), this.description(ctx.t), 1, ctx.card, 0, 'land - building');
          }
          if (selectedCards.length !== 0) {
            ctx.dropToBlocked({id: selectedCards[0].id, fromZone: ctx.zone});
            ctx.updateBlocks(ctx.card.id, [selectedCards[0].id]);
  ...
  116: {
    ...
    3: [
      ...
      },
      {
        ...
        timing: "onClick",
        execute: async function (ctx) {
          const cards = (await ctx.selectCardsFromZone((card) => card.GetType(ctx.t).includes(ctx.t('person')) && card.id !== ctx.card.id, ctx.t('playArea'), this.description(ctx.t), 2, ctx.card, 0, 'person'));
          if (cards.length === 2) {
            for (const card of cards) {
              ctx.dropToDiscard({id: card.id, fromZone: ctx.t('playArea')});
            }
        ...
        if (cards.length > 0) {
          ...
          if (card) {
            ...
            if (card.GetResources().length > 1) {
              resources = await ctx.selectResourceChoice(card.GetResources(), 1);
            }
            else {
          ...
          if (selected.length !== 0) {
            for(const card of selected) {
              ...
              while(selectedResource === null) {
                selectedResource = await ctx.selectResourceChoice(card.GetResources(), 1);
              }
  ...
  152: {
    1: [{ // Guarde Incompétent
      description: (t) => t('none'), 
      timing: "onPlayAreaUpdated", // TODO: New timing => onPlayAreaUpdated => if no enemy, discard
      execute: async function (ctx) {
      ...
      { // Chariot à Trésors
        ...
        execute: async function (ctx) {
          ...
          if (Object.keys(ctx.card.GetResources()[0]).filter(key => key !== 'fame').length > 1) {
            const choice = await ctx.selectResourceChoice([ctx.card.GetResources()[0]], 1) ?? {};
            if (! choice) {
              return false;
            }
  ...
  154: {
    1: [
      ...
      {
        description: (t) => t('effect_description_powerful_prince'), 
        timing: "played",
        execute: async function (ctx) {
          const choice = await ctx.selectResourceChoice([{ coin: 1 },   { wood: 1 }, { stone: 1 }, { sword: 1 }, { metal: 1 }, { tradegood: 1 }], 2);
          if (!choice) {
    ...
    3: [
      ...
      {
        description: (t) => t('effect_description_prince__'),
        timing: 'onClick',
        execute: async function(ctx) {
  ...
  157: {
    1: [
      { // Voleur Saboteur
        description: (t) => t('effect_description_robber_saboteur'),
        timing: "played",
        execute: async function(ctx)  {
          ...
          const resource = await ctx.selectResourceChoice(card.GetResources(), 1);
          if (! resource) {
            return false;
          }
  ...
  158: {
    1: [
      { // Moine Rebelle
        description: (t) => t('effect_description_rebellious_monk'),
        timing: "played",
        execute: async function(ctx)  {
          let selectedCards: GameCard[] = [];
          ...
          if (selectedCards.length !== 0) {
            ctx.dropToBlocked({id: selectedCards.map((c) => c.id), fromZone: ctx.zone});
  ...
  159: {
    1: [{ // Barde du Peuple
      description: (t) => t('effect_description_bard_of_the_people'),
      timing: "played",
      execute: async function(ctx)  {
        const enemies = ctx.fetchCardsInZone((c) => c.GetType(ctx.t).includes(ctx.t('enemy')), ctx.t('discard'));
        if (enemies.length > 0) {
    ...
    3: [{ // Barde Obéissant
      description: (t) => t('effect_description_obedient_bard'),
        timing: "onClick",
        execute: async function(ctx)  {
  ...
  160: {
    1: [
      { // Sauveur
        description: (t) => t('effect_description_rescuer'),
        timing: "played",
        execute: async function(ctx)  {
  ...
  163: {
    1: [{ // Camp de Druides
      description: (t) => t('effect_description_druid_camp'),
      timing: "played",
      execute: async function(ctx)  {
    ...
    2: [
      { // Sanctuaire Druidique
        description: (t) => t('effect_description_druid_sanctuary'),
        timing: "played",
        execute: async function(ctx)  {
          const stayInPlay = ctx.fetchCardsInZone((c) => c.GetEffect(ctx.t).includes(ctx.t('staysInPlay')), ctx.zone);
          await ctx.dropToDiscard({id: stayInPlay.map((c) => c.id), fromZone: ctx.zone});
    ...
    4: [{ // Forêt Reculée
      description: (t) => t('effect_description_secluded_forest'),
        timing: "played",
        execute: async function(ctx)  {
          const stayInPlay = ctx.fetchCardsInZone((c) => c.GetEffect(ctx.t).includes(ctx.t('staysInPlay')), ctx.zone);
  ...
  164: {
    3: [{ // Cérémonie
      description: (t) => t('effect_description_ceremony'),
      timing: "onClick",
      execute: async function(ctx)  {
    ...
    4: [{ // Mégalithe
      description: (t) => t('effect_description_hill_megalith'),
        timing: "onClick",
        execute: async function(ctx)  {
  ...
  165: {
    ...
    2: [
      { // Course Poursuite
        description: (t) => t('effect_description_the_chase'),
        timing: "onClick",
        execute: async function(ctx)  {
  ...
  166: {
    1: [
      ...
      {
        description: (t) => t('effect_description_sheriff'), 
        timing: "onClick",
        execute: async function (ctx) {
    ...
    3: [{ // Shérif Redouté
      description: (t) => t('effect_description_feared_sheriff'), 
      timing: "onClick",
      execute: async function (ctx) {
        const lands = ctx.fetchCardsInZone((c) => c.GetType(ctx.t).includes(ctx.t('land')), ctx.t('playArea'));
        if (lands.length === 0) {
  ...
  168: {
    2: [{ // Palissade
      description: (t) => t('effect_description_pallisade'), 
      timing: "endOfTurn",
      execute: async function (ctx) {
    ...
    3: [
      { // Muraille Avant-Poste
        description: (t) => t('effect_description_walled_outpost'), 
        timing: "endOfTurn",
        execute: async function (ctx) {
    ...
    4: [{ // Avant-Poste Forestier
      description: (t) => t('effect_description_forest_outpost'), 
      timing: "endOfTurn",
      execute: async function (ctx) {
